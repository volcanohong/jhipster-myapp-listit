  ...
  return (
    <div>
      <h2 id="location-heading">
        <Translate contentKey="listitApp.location.home.title">Locations</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="listitApp.location.home.createLabel">Create new Location</Translate>
        </Link>
      </h2>
        ...
        {locationList && locationList.length > 0 ? (
            ...
            <tbody>
              {locationList.map((location, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{location.cityId ? <Link to={`city/${location.cityId}`}>{location.cityId}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="listitApp.location.home.notFound">No Locations found</Translate>
            </div>
          )
        )}
      </div>
